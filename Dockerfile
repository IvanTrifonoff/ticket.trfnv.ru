...
ENV LC_ALL=C.UTF-8     DJANGO_SETTINGS_MODULE=production_settings
...
# Copy custom plugins
COPY plugins /pretix/plugins

RUN pip3 install -U         pip         setuptools         wheel &&     cd /pretix &&     PRETIX_DOCKER_BUILD=TRUE pip3 install         -e ".[memcached]"         gunicorn django-extensions ipython pretix-passbook &&     pip3 install -e /pretix/plugins/pretix-custom-seating &&     rm -rf ~/.cache/pip

RUN chmod +x /usr/local/bin/pretix &&     rm /etc/nginx/sites-enabled/default &&     cd /pretix/src &&     rm -f pretix.cfg &&      mkdir -p data &&     chown -R pretixuser:pretixuser /pretix /data data &&      sudo -u pretixuser make production
